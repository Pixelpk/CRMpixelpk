...
class Project_members_model extends Crud_model {
    ...
    function project_members($project_id = 0){
        $sql = "SELECT p.project_id, u.id AS user_id, u.first_name, u.last_name, tm.per_hour_pay, tm.per_hour_rate FROM project_members AS p JOIN users AS u on p.user_id = u.id LEFT JOIN team_member_job_info AS tm ON tm.user_id = u.id WHERE p.project_id = '$project_id'";
        //return $sql;
        return $this->db->query($sql);
    }
    public function user_cost($user_id = 0){
