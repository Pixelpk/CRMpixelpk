...
class Projects_model extends Crud_model {
    ...
    function project_cost($project_id = 0){
        $sql = "SELECT p.id, p.title, p.price FROM projects AS p WHERE p.id = $project_id";
        return $this->db->query($sql);
    }

    function project_time($project_id = 0){
        $sql = "SELECT * FROM project_time WHERE project_id = $project_id";
        return $this->db->query($sql);
    }
